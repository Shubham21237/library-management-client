    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800/80 pb-6">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Explore and borrow from our digital & physical collection</p>
        </div>

        {/* Feedback Alert */}
        {feedback.msg && (
          <div
            className={`p-3.5 rounded-xl text-xs font-semibold flex items-center gap-2 animate-in fade-in duration-200 ${
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="relative flex-grow">
            ...
            <input
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-11 pr-4 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition-colors text-sm"
            />
          </div>
          ...
          <button
            ...
            className={`px-4 py-2.5 rounded-xl border text-xs font-semibold flex items-center justify-center gap-2 transition-all whitespace-nowrap ${
              inStockOnly
      ...
      {loading ? (
        <div className="min-h-[40vh] flex flex-col items-center justify-center gap-3">
          ...
          <p className="text-xs text-slate-400">Loading catalog inventory...</p>
        </div>
      ) : books.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {books.map((book) => (
            <div
              key={book._id}
              className="glass-card overflow-hidden group flex flex-col hover:border-slate-700 transition-all hover:scale-[1.02] duration-300"
            >
              {/* Book Cover Image */}
              <div className="relative h-64 bg-slate-950 overflow-hidden">
                ...
                <div className="absolute top-3 right-3">
                  {book.availableCopies > 0 ? (
                    <span className="px-2.5 py-1 rounded-full bg-emerald-500/20 backdrop-blur-md border border-emerald-500/30 text-emerald-400 text-[10px] font-bold uppercase tracking-wider">
                      {book.availableCopies} Copies Available
                    </span>
                  ) : (
                    <span className="px-2.5 py-1 rounded-full bg-amber-500/20 backdrop-blur-md border border-amber-500/30 text-amber-400 text-[10px] font-bold uppercase tracking-wider flex items-center gap-1">
              ...
              <div className="p-5 flex-grow flex flex-col justify-between space-y-4">
                <div>
                  ...
                  <Link to={`/books/${book._id}`}>
                    <h3 className="text-base font-bold text-white hover:text-indigo-300 line-clamp-2 leading-snug">
                      {book.title}
                    </h3>
                  </Link>
                ...
                <div className="space-y-3 pt-3 border-t border-slate-800/80">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <Link
                      to={`/books/${book._id}`}
                      className="w-full py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 font-semibold text-xs rounded-lg text-center transition-colors"
                    >
                      Details
                    </Link>
                    ...
                    <button
                      ...
                      className={`w-full py-2 font-semibold text-xs rounded-lg transition-all flex items-center justify-center gap-1 ${
                        book.availableCopies > 0
