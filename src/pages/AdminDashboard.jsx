    ...
    <div className="max-w-[96%] mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-6">
        <div>
          ...
          <p className="text-xs text-slate-400 font-medium mt-1">Manage catalog inventory, approve borrow requests & track overdue fines</p>
        </div>

        <button
          onClick={() => setIsAddBookModalOpen(true)}
          className="px-5 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs rounded-xl transition-all shadow-lg shadow-indigo-600/30 flex items-center gap-2"
        >
      ...
      {activeTab === 'requests' && (
        <div className="glass-card overflow-hidden">
          {pendingRequests.length === 0 ? (
            <div className="p-8 text-center text-xs font-medium text-slate-400">No pending borrow requests right now.</div>
          ) : (
                ...
                <tbody className="divide-y divide-slate-800/60">
                  {pendingRequests.map(tx => (
                    <tr key={tx._id} className="hover:bg-slate-800/40">
                      <td className="p-4 font-bold text-white">{tx.user?.name}</td>
                      <td className="p-4 text-indigo-400 font-bold">{tx.book?.title}</td>
                      <td className="p-4 font-mono text-slate-400">{tx.book?.isbn}</td>
                      <td className="p-4">
      ...
      {activeTab === 'issued' && (
        <div className="glass-card overflow-hidden">
          {activeIssued.length === 0 ? (
            <div className="p-8 text-center text-xs font-medium text-slate-400">No active issued books right now.</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs text-slate-300">
                <thead className="bg-slate-950 text-slate-400 font-bold uppercase tracking-wider border-b border-slate-800">
                ...
                <tbody className="divide-y divide-slate-800/60">
                  {activeIssued.map(tx => (
                    <tr key={tx._id} className="hover:bg-slate-800/40">
                      <td className="p-4 font-bold text-white">{tx.user?.name}</td>
                      <td className="p-4 text-indigo-400 font-bold">{tx.book?.title}</td>
                      <td className="p-4 font-medium text-slate-300">{tx.dueDate ? new Date(tx.dueDate).toLocaleDateString() : 'N/A'}</td>
                      <td className="p-4">
      ...
      {activeTab === 'analytics' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="glass-card p-6 space-y-4">
            <h3 className="text-xs font-bold text-white uppercase tracking-wider">Transaction Status Breakdown</h3>
            <div className="h-64">
      ...
      {isAddBookModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md w-screen h-screen top-0 left-0">
          <div className="glass-card w-full max-w-2xl p-6 relative max-h-[90vh] overflow-y-auto shadow-2xl bg-slate-900 border-slate-800">
            ...
            <div className="p-4 rounded-xl bg-indigo-500/10 border border-indigo-500/20 mb-6 space-y-3">
              <div className="flex items-center gap-2 text-xs font-bold text-indigo-400">
                <Sparkles className="w-4 h-4 text-indigo-400" /> Feature #1: Auto-Fetch Metadata from Google Books API
              </div>
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-grow bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500"
                />
                <button
                  type="button"
            ...
            {modalFeedback.msg && (
              <div className={`p-3 rounded-xl text-xs font-bold mb-4 ${
                modalFeedback.type === 'success' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border border-rose-500/20'
              }`}>
                {modalFeedback.msg}
              </div>
            )}

            <form onSubmit={handleCreateBookSubmit} className="space-y-4 text-xs font-medium text-slate-300">
              ...
              <button
                ...
                className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl shadow-md shadow-indigo-600/30 transition-all flex items-center justify-center gap-2"
              >
